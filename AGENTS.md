...
- `pkg/sshclient`: a parser for `ssh_config` that configures Golang SSH client library accordingly; read the specs with `man ssh_config`
- `pkg/sshserver`: a parser for `sshd_config` that configures Golang SSH server library accordingly; read the specs with `man sshd_config`
## Code Style
