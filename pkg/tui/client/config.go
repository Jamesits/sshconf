...
func (t *TUI) RunQuery(queryType string) error {
	...
	case "cipher":
		algs = sshclient.AllSupportedCiphers()
	case "cipher-auth":
		for _, c := range sshclient.AllSupportedCiphers() {
			if strings.Contains(c, "gcm") || strings.Contains(c, "chacha20") {
				algs = append(algs, c)
			}
		}
	case "mac":
		algs = sshclient.AllSupportedMACs()
	case "kex":
		algs = sshclient.AllSupportedKexAlgorithms()
	case "key", "hostkeytype":
		algs = sshclient.AllSupportedHostKeyAlgorithms()
	case "key-cert":
		for _, k := range sshclient.AllSupportedHostKeyAlgorithms() {
	...
	case "key-sig", "sig":
		algs = sshclient.AllSupportedPublicKeyAlgorithms()
...
func (t *TUI) PrintConfig(opts *sshclient.Options, host, originalHost string) {
	...
	pFwd := func(key string, fwds []sshclient.Forward) {
		for _, f := range fwds {
			bind := f.BindAddress
			if bind != "" {
				bind += ":"
